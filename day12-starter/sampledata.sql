-- 무인 커피 자판기 매출 데이터 (90건)
-- 사용법: Supabase SQL Editor에 붙여넣고 Run 버튼 클릭
-- 한 사람이 여러 번, 여러 개를 구매하는 현실적인 데이터 구성
...
-- 검증 쿼리: 삽입된 데이터 확인
-- select count(*) as total_sales, sum(total_price) as total_revenue from sales;
-- select buyer_name, count(*) as purchase_count from sales group by buyer_name order by purchase_count desc;
